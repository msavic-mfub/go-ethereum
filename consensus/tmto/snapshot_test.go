...
// checkpoint creates a Tmto checkpoint signer section from the provided list
...
// sign calculates a Tmto digital signature for the given block and embeds it
...
// the account may or may not have cast a Tmto vote.
...
// Tests that Tmto signer voting is evaluated correctly for various simple and
// complex scenarios, as well as that a few special corner cases fail correctly.
func TestTmto(t *testing.T) {
	...
	for i, tt := range tests {
		...
		config.Tmto = &params.TmtoConfig{
			Period: 1,
			Epoch:  tt.epoch,
		}
		engine := New(config.Tmto, db)
		...
		// Pass all the headers through tmto and ensure tallying succeeds

...
var (
	...
	AllTmtoProtocolChanges = &ChainConfig{big.NewInt(1337), big.NewInt(0), nil, false, big.NewInt(0), common.Hash{}, big.NewInt(0), big.NewInt(0), big.NewInt(0), big.NewInt(0), big.NewInt(0), nil, nil, nil, &TmtoConfig{Period: 0, Epoch: 30000}}
...
type ChainConfig struct {
	...
	Tmto   *TmtoConfig   `json:"tmto,omitempty"`
}
...
//TmtoConfig is the consensus engine configs for time-memory trade-off.
type TmtoConfig struct {
	Period uint64 `json:"period"` // Number of seconds between blocks to enforce
	Epoch  uint64 `json:"epoch"`  // Epoch length to reset votes and checkpoint
}

// String implements the stringer interface, returning the consensus engine details.
func (c *TmtoConfig) String() string {
	return "tmto"
}
